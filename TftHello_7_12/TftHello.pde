...
void loop(void) {
  ...
  if(Serial.available() > 0){
    ...
    } else if (c == TITLE || c == ARTIST || c == ALBUM || c == GENRE){ // start of message
      msgID = c;
      theMessage = "";
  ...
  tft.setRotation(3);
  drawBoundingRect(GREEN);
  tft.fillRect(1, 1, tft.width()-3, tft.height()-3, BLACK);
  tft.setCursor(0, 0);
  tft.drawLine(0, TEXT_4_HEIGHT, tft.width()-1, TEXT_4_HEIGHT, WHITE);
